...
<body>
  ...
  <table>
        <thead>
            <tr>
                <th>Product</th>
                <th>Number of Stock Sold</th>
                <th>Revenue from Item</th>
            </tr>
        </thead>
        <tbody>
            @foreach($soldProducts as $soldProduct)
                <tr>
                    <td>{{$soldProduct[0]->product_name}}</td> 
                    <td>{{$soldProduct[1]}}</td> 
                    <td>£{{$soldProduct[1] * $soldProduct[0]->product_price}}</td> 
                </tr>
            @endforeach
        </tbody>
    </table>
  ...
  <table>
        ...
        <tbody>
            @foreach($items as $item)
                <tr>
                    <td>{{$products->where('product_id', $item->product_id)->first()->product_name}}</td> <!-- Replace with actual product name -->
                    <td>{{ $item->size_number }}</td>
                    <td>{{ $item->stock_changes_number }}</td>
                    <td>£{{$products->where('product_id', $item->product_id)->first()->product_price *  $item->stock_changes_number}}</td> <!-- Replace with actual revenue calculation -->
                    <td>{{$item->stock_changes_date}}</td>
                  </tr>
            @endforeach
        </tbody>
    </table>
...
<style>
  ...
  table {
        ...
        th {
            background-color: #f4f4f4; /* Adds a light gray background to headers */
        }
</style>